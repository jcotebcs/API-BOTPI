...
class SecureAPIKeyManager:
    ...
    # ------------------------------------------------------------------
    def _load(self) -> None:
        """Load key data from :attr:`storage_path`.

        The original implementation assumed the backing JSON file always
        contained valid data.  In practice the file may be empty or become
        corrupted (for example if a previous write was interrupted), which
        would raise :class:`json.JSONDecodeError` on load and crash the
        application.  To make the key manager more robust we now guard the
        parsing step and fall back to an empty store when invalid JSON is
        encountered.  This mirrors the behaviour of typical configuration
        loaders which treat malformed files as if they were missing.
        """
        ...
        if self.storage_path.exists():
            try:
                ...
                # Corrupt or unreadable file -> start with a clean slate.
                self._data = {}
        else:
            self._data = {}
